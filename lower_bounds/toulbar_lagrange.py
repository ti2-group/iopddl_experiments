...
def _write_results_db(
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            insert_sql = (
                "INSERT INTO toulbar_results (instance_name, ts, iteration, primal_UB, dual_LB, gap, gap_percent, best_broken)"
                " VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
            )
...
def batch_solve(
    penalties_batch, primal_UB, dual_LB, batch_scalings=None, time_limit=20
):
    with Parallel(n_jobs=8) as parallel:
        results = parallel(
            delayed(solve)(penalties_batch[i], primal_UB, time_limit=time_limit)
...
primal_UB, dual_LB, improved, penalties, diffs, best_broken = batch_solve(
    batch_penalties, primal_UB, dual_LB, time_limit=300
)
...
db_file = os.environ.get("RESULTS_DB", os.path.join(os.getcwd(), "results.db"))

for i in range(20):
    print("===> Iteration", i, flush=True)

    if i == 4:
        time_limit_per_iter = max(time_limit_per_iter, 600)

    diffs_list.append(diffs)

    m_t = momentum * m_t + (1 - momentum) * diffs
    v_t = second_momentum * v_t + (1 - second_momentum) * (diffs**2)
    m_t_hat = m_t / (1 - momentum ** (i + 1))
    v_t_hat = v_t / (1 - second_momentum ** (i + 1))
    ...
    for n_idx, node in enumerate(problem["nodes"]):
        diff_sum = sum(
            diffs[i] if diffs[i] < 0 else diffs[i] * 10 for i in sets_by_var[n_idx]
        )
        if diff_sum < 0 and len(set(sets_by_var[n_idx]).intersection(best_broken)) > 0:
            wrong_diffs += 1
    print("Wrong diffs", wrong_diffs, flush=True)
    ...
    for scale in batch_scalings:
        new_penalty = penalties + step * scale * m_t_hat / (np.sqrt(v_t_hat) + 1e-8)
        new_penalty = penalties.copy()
        new_penalty[m_t > 0] += step * scale * 1 * m_t[m_t > 0]
        new_penalty[m_t <= 0] += step * scale * m_t[m_t <= 0] 
    ...
    if not improved:
        ...
        time_limit_per_iter *= 2
        time_limit_per_iter = min(time_limit_per_iter, 7200)
    ...
    try:
        with open(f"{chars[instance_id]}_{instance_id}_{i}_penalties.npy", "wb") as f:
            np.save(f, penalties)
        with open(f"{chars[instance_id]}_{instance_id}_{i}_diffs.npy", "wb") as f:
            np.save(f, diffs)
    except Exception as e:
        print("Failed to save penalties:", e, flush=True)
    try:
        _write_results_db(db_file, [results[-1]], instance_name=instance_name)
    except Exception as e:
        print("Failed to write results to DB:", e, flush=True)

D = np.stack(diffs_list)  # shape (num_calls, m_constraints)
avg_violation = np.mean(
    np.maximum(0, D), axis=0
)  # average positive violation per constraint
freq_viol = np.mean(D > 1e-9, axis=0)  # fraction of calls where constraint violated

# show top offenders
idx = np.argsort(-avg_violation)
idx_for_freq = np.argsort(-freq_viol)
print("Top constraints by avg violation:", idx[:20], flush=True)
print(
    "Top constraints by frequency:",
    idx_for_freq[:20],
    freq_viol[idx_for_freq[:20]],
    flush=True,
)


# Persist results to sqlite database. If another process holds the DB, retry a few times.

try:
    _write_results_db(db_file, results, instance_name=instance_name)
except Exception as e:
    print("Failed to write results to DB:", e, flush=True)
