...
bool EnumeratedVariable::elimVar(ConstraintLink xylink, ConstraintLink xzlink)
{
    ...
    auto c1 = static_cast<BinaryConstraint*>(xylink.constr);
    auto c2 = static_cast<BinaryConstraint*>(xzlink.constr);
    const int is_xzero = (xylink.scopeIndex == 0);
    const int is_zzero = (xzlink.scopeIndex == 0);

    constexpr int opt = 0;

    if constexpr (opt == 0){
//        std::vector<unsigned int> c1RowOffset(c1->sizeX);
//        std::vector<unsigned int> c2RowOffset(c2->sizeX);
//        for (unsigned int x = 0; x < c1->sizeX; ++x) {
//            c1RowOffset[x] = x * c1->sizeY;
//        }
//        for (unsigned int x = 0; x < c2->sizeX; ++x) {
//            c2RowOffset[x] = x * c2->sizeY;
//        }
//

// Cache pointers to the underlying arrays to avoid repeated pointer indirection.
        const auto c1C      = c1->costs.data();
        const auto c2C      = c2->costs.data();
        const auto c1dX_ptr = c1->deltaCostsX.data();
        const auto c1dY_ptr = c1->deltaCostsY.data();
        const auto c2dX_ptr = c2->deltaCostsX.data();
        const auto c2dY_ptr = c2->deltaCostsY.data();

        std::vector<unsigned int> xvals;
        for (auto it = begin(); it != end(); ++it) {
            xvals.push_back(*it);
        }
        size_t totalX = xvals.size();

        if (is_xzero) {
            if (is_zzero) {
                // --- Branch: is_xzero = 1, is_zzero = 1 ---
                // c1 is indexed as [x][y] and c2 as [x][z].
                for (auto itery = y->begin(); itery != y->end(); ++itery) {
                    const unsigned int yVal = *itery;
                    for (auto iterz = z->begin(); iterz != z->end(); ++iterz) {
                        const unsigned int zVal = *iterz;
                        Cost mincost = MAX_COST;
                        size_t i = 0;
                        // Unroll inner loop by factor 4.
                        for (; i + 3 < totalX; i += 4) {
                            unsigned int x0 = xvals[i];
                            unsigned int row1 = c1->sizeY * x0;
                            Cost res1 = c1C[row1 + yVal] - (c1dX_ptr[x0] + c1dY_ptr[yVal]);
                            unsigned int row2 = c2->sizeY * x0;
                            Cost res2 = c2C[row2 + zVal] - (c2dX_ptr[x0] + c2dY_ptr[zVal]);
                            Cost curcost = costs[x0] - deltaCost + res1 + res2;

                            unsigned int x1 = xvals[i + 1];
                            unsigned int _row1 = c1->sizeY * x1;
                            Cost _res1 = c1C[_row1 + yVal] - (c1dX_ptr[x1] + c1dY_ptr[yVal]);
                            unsigned int _row2 = c2->sizeY * x1;
                            Cost _res2 = c2C[_row2 + zVal] - (c2dX_ptr[x1] + c2dY_ptr[zVal]);
                            Cost _curcost = costs[x1] - deltaCost + _res1 + _res2;

                            unsigned int x2 = xvals[i + 2];
                            unsigned int __row1 = c1->sizeY * x2;
                            Cost __res1 = c1C[__row1 + yVal] - (c1dX_ptr[x2] + c1dY_ptr[yVal]);
                            unsigned int __row2 = c2->sizeY * x2;
                            Cost __res2 = c2C[__row2 + zVal] - (c2dX_ptr[x2] + c2dY_ptr[zVal]);
                            Cost __curcost = costs[x2] - deltaCost + __res1 + __res2;

                            unsigned int x3 = xvals[i + 3];
                            unsigned int ___row1 = c1->sizeY * x3;
                            Cost ___res1 = c1C[___row1 + yVal] - (c1dX_ptr[x3] + c1dY_ptr[yVal]);
                            unsigned int ___row2 = c2->sizeY * x3;
                            Cost ___res2 = c2C[___row2 + zVal] - (c2dX_ptr[x3] + c2dY_ptr[zVal]);
                            Cost ___curcost = costs[x3] - deltaCost + ___res1 + ___res2;

                            bool is_min = curcost < _curcost;
                            bool _is_min = __curcost < ___curcost;
                            curcost = curcost * is_min + _curcost * !is_min;
                            __curcost = __curcost * _is_min + ___curcost * !_is_min;
                            bool __is_min = curcost < __curcost;
                            curcost = curcost * __is_min + __curcost * !__is_min;
                            is_min = curcost < mincost;
                            mincost = curcost * is_min + mincost * !is_min;

//                        if (curcost < mincost) mincost = curcost;
//                        if (_curcost < mincost) mincost = _curcost;
//                        if (__curcost < mincost) mincost = __curcost;
//                        if (___curcost < mincost) mincost = ___curcost;
                        }
                        // Process remaining x–values.
                        for (; i < totalX; i++) {
                            unsigned int xVal = xvals[i];
                            unsigned int row1 = c1->sizeY * xVal;
                            Cost res1 = c1C[row1 + yVal] - (c1dX_ptr[xVal] + c1dY_ptr[yVal]);
                            unsigned int row2 = c2->sizeY * xVal;
                            Cost res2 = c2C[row2 + zVal] - (c2dX_ptr[xVal] + c2dY_ptr[zVal]);
                            Cost curcost = costs[xVal] - deltaCost + res1 + res2;
                            bool is_min = curcost < mincost;
                            mincost = curcost * is_min + mincost * !is_min;
                        }
                        // Update negcost and flag.
                        if (mincost < negcost) {
                            if (negcost < MAX_COST) flag = true;
                            negcost = mincost;
                        } else if (mincost > negcost)
                            flag = true;
                        yznew->setcost(yVal, zVal, mincost);
                    }
                }
            } else {
                // --- Branch: is_xzero = 1, is_zzero = 0 ---
                // c1 is indexed as [x][y] and c2 as [z][x] (i.e. for c2, the row is determined by z).
                for (auto itery = y->begin(); itery != y->end(); ++itery) {
                    const unsigned int yVal = *itery;
                    for (auto iterz = z->begin(); iterz != z->end(); ++iterz) {
                        const unsigned int zVal = *iterz;
                        Cost mincost = MAX_COST;
                        size_t i = 0;
                        // Precompute the constant row index for c2 (since c2 is indexed [z][x]):
                        unsigned int row2_const = zVal * c2->sizeY;
                        for (; i + 3 < totalX; i += 4) {
                            unsigned int x0 = xvals[i];
                            unsigned int row1 = c1->sizeY * x0;
                            Cost res1 = c1C[row1 + yVal] - (c1dX_ptr[x0] + c1dY_ptr[yVal]);
                            Cost res2 = c2C[row2_const + x0] - (c2dX_ptr[zVal] + c2dY_ptr[x0]);
                            Cost curcost = costs[x0] - deltaCost + res1 + res2;

                            unsigned int x1 = xvals[i + 1];
                            unsigned int _row1 = c1->sizeY * x1;
                            Cost _res1 = c1C[_row1 + yVal] - (c1dX_ptr[x1] + c1dY_ptr[yVal]);
                            Cost _res2 = c2C[row2_const + x1] - (c2dX_ptr[zVal] + c2dY_ptr[x1]);
                            Cost _curcost = costs[x1] - deltaCost + _res1 + _res2;

                            unsigned int x2 = xvals[i + 2];
                            unsigned int __row1 = c1->sizeY * x2;
                            Cost __res1 = c1C[__row1 + yVal] - (c1dX_ptr[x2] + c1dY_ptr[yVal]);
                            Cost __res2 = c2C[row2_const + x2] - (c2dX_ptr[zVal] + c2dY_ptr[x2]);
                            Cost __curcost = costs[x2] - deltaCost + __res1 + __res2;

                            unsigned int x3 = xvals[i + 3];
                            unsigned int ___row1 = c1->sizeY * x3;
                            Cost ___res1 = c1C[___row1 + yVal] - (c1dX_ptr[x3] + c1dY_ptr[yVal]);
                            Cost ___res2 = c2C[row2_const + x3] - (c2dX_ptr[zVal] + c2dY_ptr[x3]);
                            Cost ___curcost = costs[x3] - deltaCost + ___res1 + ___res2;
...
//                        if (___curcost < mincost) mincost = ___curcost;
                        ...
                        for (; i < totalX; i++) {
                            ...
                            unsigned int row1 = c1->sizeY * xVal;
                            Cost res1 = c1C[row1 + yVal] - (c1dX_ptr[xVal] + c1dY_ptr[yVal]);
                            Cost res2 = c2C[row2_const + xVal] - (c2dX_ptr[zVal] + c2dY_ptr[xVal]);
                            Cost curcost = costs[xVal] - deltaCost + res1 + res2;
                            bool is_min = curcost < mincost;
                            mincost = curcost * is_min + mincost * !is_min;
                        }
            ...
            if (is_zzero) {
                // --- Branch: is_xzero = 0, is_zzero = 1 ---
                // c1 is indexed as [y][x] and c2 as [x][z].
                for (auto itery = y->begin(); itery != y->end(); ++itery) {
                    const unsigned int yVal = *itery;
                    // Precompute the constant row index for c1:
                    unsigned int row1_const = yVal * c1->sizeY;
                    for (auto iterz = z->begin(); iterz != z->end(); ++iterz) {
                        const unsigned int zVal = *iterz;
                        Cost mincost = MAX_COST;
                        size_t i = 0;
                        for (; i + 3 < totalX; i += 4) {
                            unsigned int x0 = xvals[i];
                            Cost res1 = c1C[row1_const + x0] - (c1dX_ptr[yVal] + c1dY_ptr[x0]);
                            unsigned int row2 = c2->sizeY * x0;
                            Cost res2 = c2C[row2 + zVal] - (c2dX_ptr[x0] + c2dY_ptr[zVal]);
                            Cost curcost = costs[x0] - deltaCost + res1 + res2;
                            ...
                            unsigned int x1 = xvals[i + 1];
                            Cost _res1 = c1C[row1_const + x1] - (c1dX_ptr[yVal] + c1dY_ptr[x1]);
                            unsigned int _row2 = c2->sizeY * x1;
                            Cost _res2 = c2C[_row2 + zVal] - (c2dX_ptr[x1] + c2dY_ptr[zVal]);
                            Cost _curcost = costs[x1] - deltaCost + _res1 + _res2;

                            unsigned int x2 = xvals[i + 2];
                            Cost __res1 = c1C[row1_const + x2] - (c1dX_ptr[yVal] + c1dY_ptr[x2]);
                            unsigned int __row2 = c2->sizeY * x2;
                            Cost __res2 = c2C[__row2 + zVal] - (c2dX_ptr[x2] + c2dY_ptr[zVal]);
                            Cost __curcost = costs[x2] - deltaCost + __res1 + __res2;

                            unsigned int x3 = xvals[i + 3];
                            Cost ___res1 = c1C[row1_const + x3] - (c1dX_ptr[yVal] + c1dY_ptr[x3]);
                            unsigned int ___row2 = c2->sizeY * x3;
                            Cost ___res2 = c2C[___row2 + zVal] - (c2dX_ptr[x3] + c2dY_ptr[zVal]);
                            Cost ___curcost = costs[x3] - deltaCost + ___res1 + ___res2;
...
//                            if (___curcost < mincost) mincost = ___curcost;
                        ...
                        for (; i < totalX; i++) {
                            unsigned int xVal = xvals[i];
                            Cost res1 = c1C[row1_const + xVal] - (c1dX_ptr[yVal] + c1dY_ptr[xVal]);
                            unsigned int row2 = c2->sizeY * xVal;
                            Cost res2 = c2C[row2 + zVal] - (c2dX_ptr[xVal] + c2dY_ptr[zVal]);
                            Cost curcost = costs[xVal] - deltaCost + res1 + res2;
                            bool is_min = curcost < mincost;
                ...
                // --- Branch: is_xzero = 0, is_zzero = 0 ---
                // c1 is indexed as [y][x] and c2 as [z][x].
                for (auto itery = y->begin(); itery != y->end(); ++itery) {
                    const unsigned int yVal = *itery;
                    unsigned int row1_const = yVal * c1->sizeY;
                    for (auto iterz = z->begin(); iterz != z->end(); ++iterz) {
...
// === Process According to is_xzero and is_zzero ===
//        if (is_xzero) {
//            if (is_zzero) {
//                // --- Branch: is_xzero = 1, is_zzero = 1 ---
//                // c1 is indexed as [x][y] and c2 as [x][z].
...
//                        for (; i + 3 < totalX; i += 4) {
//                            unsigned int x0 = xvals[i];
//                            unsigned int row1 = c1RowOffset[x0];
//                            Cost res1 = c1C[row1 + yVal] - (c1dX_ptr[x0] + c1dY_ptr[yVal]);
//                            unsigned int row2 = c2RowOffset[x0];
//                            Cost res2 = c2C[row2 + zVal] - (c2dX_ptr[x0] + c2dY_ptr[zVal]);
//                            Cost curcost = costs[x0] - deltaCost + res1 + res2;
//
//                            unsigned int x1 = xvals[i + 1];
//                            unsigned int _row1 = c1RowOffset[x1];
//                            Cost _res1 = c1C[_row1 + yVal] - (c1dX_ptr[x1] + c1dY_ptr[yVal]);
//                            unsigned int _row2 = c2RowOffset[x1];
//                            Cost _res2 = c2C[_row2 + zVal] - (c2dX_ptr[x1] + c2dY_ptr[zVal]);
//                            Cost _curcost = costs[x1] - deltaCost + _res1 + _res2;
//
//                            unsigned int x2 = xvals[i + 2];
//                            unsigned int __row1 = c1RowOffset[x2];
//                            Cost __res1 = c1C[__row1 + yVal] - (c1dX_ptr[x2] + c1dY_ptr[yVal]);
//                            unsigned int __row2 = c2RowOffset[x2];
//                            Cost __res2 = c2C[__row2 + zVal] - (c2dX_ptr[x2] + c2dY_ptr[zVal]);
//                            Cost __curcost = costs[x2] - deltaCost + __res1 + __res2;
//
//                            unsigned int x3 = xvals[i + 3];
//                            unsigned int ___row1 = c1RowOffset[x3];
//                            Cost ___res1 = c1C[___row1 + yVal] - (c1dX_ptr[x3] + c1dY_ptr[yVal]);
//                            unsigned int ___row2 = c2RowOffset[x3];
//                            Cost ___res2 = c2C[___row2 + zVal] - (c2dX_ptr[x3] + c2dY_ptr[zVal]);
//                            Cost ___curcost = costs[x3] - deltaCost + ___res1 + ___res2;
//
//                            bool is_min = curcost < _curcost;
...
//                        for (; i < totalX; i++) {
//                            unsigned int xVal = xvals[i];
//                            unsigned int row1 = c1RowOffset[xVal];
//                            Cost res1 = c1C[row1 + yVal] - (c1dX_ptr[xVal] + c1dY_ptr[yVal]);
//                            unsigned int row2 = c2RowOffset[xVal];
//                            Cost res2 = c2C[row2 + zVal] - (c2dX_ptr[xVal] + c2dY_ptr[zVal]);
//                            Cost curcost = costs[xVal] - deltaCost + res1 + res2;
//                            bool is_min = curcost < mincost;
//                            mincost = curcost * is_min + mincost * !is_min;
...
//        }
        ...
        const auto c1costs = c1->costs.data();
        const auto c1dX = c1->deltaCostsX.data();
        const auto c1dY = c1->deltaCostsY.data();
        const auto c2costs = c2->costs.data();
        const auto c2dX = c2->deltaCostsX.data();
        const auto c2dY = c2->deltaCostsY.data();


        if (is_xzero) {
            if (is_zzero) {
                // case (is_xzero = 1, is_zzero = 1):
                for (auto itery = y->begin(); itery != y->end(); ++itery) {
                    for (auto iterz = z->begin(); iterz != z->end(); ++iterz) {
                        Cost mincost = MAX_COST;
                        for (auto iterx = begin(); iterx != end(); ++iterx) {
                            // c1 is indexed [x][y]
                            unsigned int ix1 = *iterx;
                            unsigned int iy1 = *itery;
                            Cost res1 = c1costs[ix1 * c1->sizeY + iy1] - (c1dX[ix1] + c1dY[iy1]);

                            // c2 is indexed [x][z]
                            unsigned int ix2 = *iterx;
                            unsigned int iy2 = *iterz;
                            Cost res2 = c2costs[ix2 * c2->sizeY + iy2] - (c2dX[ix2] + c2dY[iy2]);

                            Cost curcost = costs[*iterx] - deltaCost + res1 + res2;
                ...
                for (auto itery = y->begin(); itery != y->end(); ++itery) {
                    for (auto iterz = z->begin(); iterz != z->end(); ++iterz) {
                        ...
                        for (auto iterx = begin(); iterx != end(); ++iterx) {
                            ...
                            // c2 is indexed [z][x]
                            unsigned int ix2 = *iterz;
                            unsigned int iy2 = *iterx;
                            Cost res2 = c2costs[ix2 * c2->sizeY + iy2] - (c2dX[ix2] + c2dY[iy2]);
            ...
            if (is_zzero) {
                // case (is_xzero = 0, is_zzero = 1):
                for (auto itery = y->begin(); itery != y->end(); ++itery) {
                    for (auto iterz = z->begin(); iterz != z->end(); ++iterz) {
                        Cost mincost = MAX_COST;
                        for (auto iterx = begin(); iterx != end(); ++iterx) {
                            // c1 is indexed [y][x]
                            unsigned int ix1 = *itery;
                            unsigned int iy1 = *iterx;
                            Cost res1 = c1costs[ix1 * c1->sizeY + iy1] - (c1dX[ix1] + c1dY[iy1]);
        ...
        for (iterator itery = y->begin(); itery != y->end(); ++itery) {
            for (iterator iterz = z->begin(); iterz != z->end(); ++iterz) {
                ...
                for (iterator iter = begin(); iter != end(); ++iter) {
                    ...
                    unsigned int ix1 = *iter * is_xzero + *itery * !is_xzero;
                    unsigned int iy1 = *itery * is_xzero + *iter * !is_xzero;
                    Cost res1 = c1->costs[ix1 * (c1->sizeY) + iy1] - (c1->deltaCostsX[ix1] + c1->deltaCostsY[iy1]);

                    unsigned int ix2 = *iter * is_zzero + *iterz * !is_zzero;
                    unsigned int iy2 = *iterz * is_zzero + *iter * !is_zzero;
                    Cost res2 = c2->costs[ix2 * (c2->sizeY) + iy2] - (c2->deltaCostsX[ix2] + c2->deltaCostsY[iy2]);

                    Cost curcost = costs[*iter] - deltaCost + res1 + res2;
        ...
        for (iterator itery = y->begin(); itery != y->end(); ++itery) {
            for (iterator iterz = z->begin(); iterz != z->end(); ++iterz) {
                Cost mincost = MAX_COST;
                for (iterator iter = begin(); iter != end(); ++iter) {
                    Cost curcost = getCost(*iter) + getBinaryCost(xylink, *iter, *itery) +
